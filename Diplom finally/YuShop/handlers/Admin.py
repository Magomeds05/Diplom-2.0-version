...
async def start(message: types.Message):
    if message.from_user.id in config.admins:
        await message.answer(texts.text.start_admin, parse_mode="HTML", reply_markup=AdminPanel)
...
➖➖➖➖➖➖➖➖➖'''
    ...
    await call.answer()
    for (id, time,) in database.get_users():
...
async def statick(call: types.CallbackQuery):
    await call.message.edit_text(texts.text.statistick(int(database.count())), parse_mode="HTML",
...
async def block(call: types.CallbackQuery):
    await call.message.answer(texts.text.ban_from_admin_start, parse_mode="HTML",
...
async def ban1(message, state):
    ...
    text = message.text
    if text == '/cancel':
        await message.answer(texts.text.ban_from_admin_cancel, reply_markup=AdminPanel)
        await state.finish()
        return
    ...
    if text.isdigit():
        id = int(text)
        try:
            await bot.send_message(id, texts.text.ban)
        except Exception as e:
            print(e)
        database.block(id)
        await message.answer(texts.text.ban_from_admin_finaly, parse_mode='HTML', reply_markup=AdminPanel)
        await state.finish()
    else:
        await message.answer(texts.text.ban_from_admin_except, parse_mode='HTML')


async def back_admin(call: types.CallbackQuery):
    await call.message.edit_text(texts.text.start_admin, parse_mode="HTML", reply_markup=AdminPanel)
    await call.answer()